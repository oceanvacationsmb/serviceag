...
reviewBackdrop.addEventListener('click', closeReviewPopup);
...
reviewNewAgreementBtn.addEventListener('click', async () => {
  ...
  await showReview(ownerPayload, adminPayload, 'new');
});
...
reviewLoadedAgreementBtn.addEventListener('click', async () => {
  ...
  await showReview(getAgreementPayloadFromForm(), getAdminPayloadFromForm(), 'loaded');
});
deleteBtn.addEventListener('click', deleteAgreement);
signDownloadBtn.addEventListener('click', signDownloadPdf);
...
async function showReview(ownerPayload, adminPayload, mode) {
  try {
    if (!ownerPayload.ach_start_date && ownerPayload.effective_date) {
      ownerPayload.ach_start_date = calculateAchStartDate(ownerPayload.effective_date);
    }
...
async function signDownloadPdf() {
  if (!selectedRecord) {
    adminStatus.textContent = 'Choose an agreement first.';
    return;
  }
